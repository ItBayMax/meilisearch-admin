<template>
  <SearchLayout ref="searchLayout">
    ...
    <template #main-content>
      ...
      <Empty v-else :title="settingsStore.t('loading')" description="" />
      ...
      <Modal v-model:visible="showDeleteModal" :title="settingsStore.t('deleteDocument')" size="sm">
        <p class="text-gray-300">{{ settingsStore.t('deleteConfirm') }}</p>
        <template #footer>
          <div class="flex justify-end space-x-3">
...
const generateCurlCommand = () => {
  const query = buildMeilisearchQuery()
  const meilisearchHost = localStorage.getItem('MEILISEARCH_HOST') || 'http://localhost:7700'
  const apiKey = localStorage.getItem('MEILISEARCH_API_KEY') || ''
  const meilisearchUrl = `${meilisearchHost}/indexes/${indexId.value}/search`
  ...
  ${authHeader}-d '${JSON.stringify(query, null, 2)}'`
}
...
const generatePostmanCollection = () => {
  ...
  return {
    info: {
      name: `Meilisearch Search - ${indexId.value}`,
    ...
    item: [
      {
        ...
        request: {
          ...
          url: {
            ...
            path: ['indexes', indexId.value, 'search']
          }
        }
      }
    ]
...
const downloadQueryBody = async () => {
  try {
    ...
    a.href = url
    a.download = `meilisearch-query-${indexId.value}.json`
...
const exportAsPostman = async () => {
  try {
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `meilisearch-${indexId.value}-search.postman_collection.json`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    URL.revokeObjectURL(url)
...
const loadSettings = () => {
  if (settings.value) {
    sortableAttributes.value = settings.value.sortableAttributes || []
    const fa = settings.value.filterableAttributes || []
...
const performSearch = async () => {
  ...
  try {
    ...
    if (result.data?.hits?.length > 0) {
      allColumns.value = Object.keys(result.data.hits[0]).filter(k => !k.startsWith('_'))
      if (selectedColumns.value.length === 0) {
        selectedColumns.value = allColumns.value.slice(0, 6)
      }
    }
  } catch (err) {
    console.error('Search failed:', err)
    results.value = { hits: [], error: err.message }
  } finally {
    loading.value = false
  }
}
...
const saveDocument = async () => {
  ...
  try {
    ...
    await indexApi.addDocuments(projectId.value, indexId.value, [doc])
...
const deleteDocument = async () => {
  ...
  deleting.value = true
  try {
    const primaryKey = settings.value?.primaryKey || 'id'
    const docId = documentToDelete.value[primaryKey]
    if (docId) {
      await indexApi.deleteDocument(projectId.value, indexId.value, docId)
...
// Watch for index changes and reset filter states
const currentIndexId = ref(indexId)
watch(() => route.params.indexId, (newIndexId, oldIndexId) => {
  if (newIndexId !== oldIndexId) {
    // Reset filter conditions and selections when index changes
    filterConditions.value = [{ attribute: '', operator: '', value: '' }]
    Object.keys(filters).forEach(key => delete filters[key])
    allColumns.value = []
    selectedColumns.value = []
    currentIndexId.value = newIndexId
  }
})
